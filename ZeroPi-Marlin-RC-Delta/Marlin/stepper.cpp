...
FORCE_INLINE unsigned short calc_timer(unsigned short step_rate) {
  ...
  timer = (unsigned short)(STEPPER_TIMER_CLK/step_rate);//MYSERIAL.println(timer);
  if(timer < 100) { timer = 100; MYSERIAL.print(MSG_STEPPER_TOO_HIGH); MYSERIAL.println(step_rate); }//(20kHz this should never happen)
  return timer;
}
...
//ISR(TIMER1_COMPA_vect) {
  ...
  if (current_block != NULL) {
    ...
    //STEPPER_TIMER_OCR = (OCR1A < (TCNT1 + 16)) ? (TCNT1 + 16) : OCR1A;
	STEPPER_TIMER_OCR = (STEPPER_TIMER_OCR < (STEPPER_TIMER_CNT + 16)) ? (STEPPER_TIMER_CNT + 16) : STEPPER_TIMER_OCR;
